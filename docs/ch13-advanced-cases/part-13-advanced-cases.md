...
> 🎯 目标：通过复杂真实场景的端到端案例，展示如何在大型工程中高效使用 Agent。本章聚焦三类进阶模式：长任务管理、多 Agent 协作、以及让系统越用越好的演化机制。
...
flowchart TB
    ...
    style Init fill:#2d2d2d,stroke:#61dafb,color:#fff
    style Done fill:#2d2d2d,stroke:#98c379,color:#fff
    style Check fill:#2d2d2d,stroke:#e5c07b,color:#fff
```
...
> ⚠️ **反模式**：让 Agent 在一次对话里完成全部 40 个文件的迁移。对话越长，越容易忘记前面的约束，迁移质量会随轮次下降。
...
flowchart TB
    ...
    style Planner fill:#2d2d2d,stroke:#61dafb,color:#fff
    style Reducer fill:#2d2d2d,stroke:#c678dd,color:#fff
    style W1 fill:#2d2d2d,stroke:#98c379,color:#fff
    style W2 fill:#2d2d2d,stroke:#98c379,color:#fff
    style W3 fill:#2d2d2d,stroke:#98c379,color:#fff
```
...
---
flowchart LR
    G["⚙️ Generate<br/>执行任务"] --> E["📊 Evaluate<br/>评估质量"]
    E --> P["✍️ Prune & Promote<br/>提炼可靠经验"]
    P --> A["🚀 Apply<br/>写入 CLAUDE.md / Skill"]
    A --> G

    style G fill:#2d2d2d,stroke:#98c379,color:#fff
    style E fill:#2d2d2d,stroke:#e5c07b,color:#fff
    style P fill:#2d2d2d,stroke:#61dafb,color:#fff
    style A fill:#2d2d2d,stroke:#c678dd,color:#fff
```
...
> ⚠️ **关键警告**：经验演化系统的最大风险是把**错误经验固化进长期记忆**。必须有人工或评测机制过滤，不能让 Agent 自动更新自己的长期规则。
...
flowchart LR
    ...
    style Decide fill:#2d2d2d,stroke:#e06c75,color:#fff
    style JSON fill:#2d2d2d,stroke:#e06c75,color:#fff
```
...
flowchart TB
    ...
    style Guard1 fill:#1a1a2e,stroke:#61dafb
    style Guard2 fill:#1a1a2e,stroke:#e5c07b
    style Guard3 fill:#1a1a2e,stroke:#98c379
```
...
> 💡 **延伸阅读**：工具调用 + 上下文缓存的底层机制，见 [附录：Memory 与上下文工程详解](../ch02-concepts/reference-memory-and-context.md)。
...
flowchart LR
    ...
    style I fill:#2d2d2d,stroke:#61dafb,color:#fff
    style V fill:#2d2d2d,stroke:#e5c07b,color:#fff
    style R fill:#2d2d2d,stroke:#98c379,color:#fff
```
...
flowchart TB
    ...
    style PAUSE fill:#2d2d2d,stroke:#e06c75,color:#fff
    style A fill:#2d2d2d,stroke:#61dafb,color:#fff
    style HUMAN fill:#2d2d2d,stroke:#e5c07b,color:#fff
```
