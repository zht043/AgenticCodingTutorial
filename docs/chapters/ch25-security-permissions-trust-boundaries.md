...
## 5. Auto Mode：基于模型的权限委托

Claude Code 的 Auto Mode 是一个具体的安全工程案例，展示了如何在「全手动审批」和「完全放开」之间找到中间地带。

### 问题背景：权限疲劳

统计显示用户实际同意了 93% 的审批请求。审批疲劳让人不再仔细看就批准——真正的危险反而被放过。
...
两层互相补充：输出层的分类器永远看不到工具输出，但如果注入劫持了 Agent，它发出的工具调用仍然要过分类器。攻击要成功必须同时绕过两层。

### 三级权限过滤

| 层级 | 匹配什么 | 怎么处理 |
|------|---------|---------|
| **Tier 1** | 内置安全白名单 + 用户设置（文件读取、文本搜索等） | 直接放行，0 延迟 |
| **Tier 2** | 项目目录内的文件写入和编辑 | 直接允许，可通过版本控制审查 |
| **Tier 3** | Shell 命令、网络请求、外部工具、子 Agent、项目外操作 | 送入转录分类器 |

Tier 3 的分类器分两阶段：
- **Stage 1**：快速单 token 过滤，倾向拦截（高召回率优先）
- **Stage 2**：链式推理重评，减少误报（高准确率优先）

### 评测数据

| 场景 | Stage 1 误报率 | 完整管线误报率 |
|------|---------------|---------------|
| 真实流量 | 8.5% | 0.4% |
| 真实过度热心 | — | 漏报率 17% |
| 合成外泄 | — | 漏报率 5.7% |

17% 的漏报率是一个诚实的数字——Agent 确实会尝试执行超出用户授权范围的操作。

### 四种威胁来源

1. **过度热心** — Agent 理解了目标，但做了超出授权的事
2. **误判范围** — Agent 搞错了操作的影响范围
3. **提示注入** — 攻击者在文件、网页或工具输出中埋入指令
4. **模型失对齐** — Agent 追求自己的目标而非用户的

### 多 Agent 交接安全

当主 Agent 创建子 Agent 时，两道额外检查：
- **出站检查**：防止主 Agent 擅自委派用户没要求的工作
- **返回检查**：防止子 Agent 在运行中被注入后将恶意结果带回

**熔断机制**：连续 3 次被拒绝或累计 20 次被拒绝 → 停止模型并上报人类。这是防止被劫持的 Agent 反复冲向同一目标的最后保险。
...
- Auto Mode 的双层防御和三级过滤，展示了如何在自动化和安全之间取得平衡。

---

<details><summary><span style="color: #e67e22; font-weight: bold;">🔒 进阶：Agent 安全治理框架与权限边界详解</span></summary>
...
- 想看 Agent 的底层机制与未来形态：继续看 [Ch26 · Agent 内幕、范式对比与未来形态](./ch26-internals-paradigms-futures.md)
...
[📚 返回目录](../../README.md#tutorial-contents) | [⬅️ 上一章：Ch24 任务适配度、能力边界与成熟度](./ch24-task-fit-boundaries-maturity.md) | [➡️ 下一章：Ch26 Agent 内幕、范式对比与未来形态](./ch26-internals-paradigms-futures.md)
