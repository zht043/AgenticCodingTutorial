...
### SDD（Spec Driven Development）的冷思考

SDD 的核心主张是：既然 AI 能写代码了，人类只需要写文档和需求描述就行。这个想法听起来很诱人，但有几个深层问题值得注意。

**精确性悖论**

> 代码本身就是最精确的规格说明。编程语言是人类花了几十年打磨出来的、专门用来无歧义描述计算过程的形式化语言。而自然语言天生就是模糊的。
...
- 冻结金额算不算？
- 「不足」是严格小于还是小于等于？
- 「拒绝」是静默失败、返回错误码、还是抛异常？
- 并发场景下检查余额和扣款之间有别的交易进来怎么办？

每一个你觉得「显然」的地方，都藏着歧义。

**瀑布流陷阱**

SDD 有一个天然倾向：在开始之前做一个大而全的规划，试图用一份详细的 spec 覆盖所有情况。这和软件行业几十年来用血泪教训换来的经验直接冲突：

> 软件开发是一个发现过程，不是一个制造过程。

**工程决策无法写进 spec**

真正决定软件系统质量的，往往不是某个功能怎么实现，而是架构层面的权衡：用事件驱动还是请求响应？数据要强一致还是最终一致？错误处理是快速失败还是优雅降级？这些决策很难用自然语言 spec 精确表达。

**更现实的做法：分层混合**

- 高层的意图和目标用自然语言写
- 关键的设计决策用结构化方式记录
- 具体的实现代码让 AI 生成但人类必须审查
- 测试用例由人类写关键 case，让 AI 补充边界 case
- 不要试图让 spec 覆盖一切，保持小粒度增量迭代
