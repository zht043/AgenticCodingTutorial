...
> 📌 **章节职责**：Ch08 讲“具体该按哪些命令操作”；本章讲“为什么这些动作构成控制面，以及你该如何设计长期协作策略”。
>
> 📖 **阅读顺序**：如果你还不熟 `/clear`、`/compact`、`--resume` 这些具体操作，先回 Ch08；本章默认你已经会用，再继续讨论什么时候该用、为什么这样用。
...
这里关注的不是“模板长什么样”，而是**哪些内容应该被提升到控制面**。如果你需要一份从零可抄的项目级样例，回看 Ch06 会更顺手。
...
Ch08 已经讲过具体命令和使用时机，这里只从控制面角度回答一个问题：**什么时候应该保留上下文，什么时候应该主动丢弃上下文。**
