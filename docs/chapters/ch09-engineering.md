...
- [7. 🏗️ Vibe → Spec → Harness：工程成熟度演进路径](#7-️-vibe--spec--harness工程成熟度演进路径)
...
flowchart LR
    ...
    style Plan fill:#2d2d2d,stroke:#61dafb
    style Design fill:#2d2d2d,stroke:#98c379
    style Code fill:#2d2d2d,stroke:#e5c07b
    style Test fill:#2d2d2d,stroke:#c678dd
    style Review fill:#2d2d2d,stroke:#e06c75
    style Deploy fill:#2d2d2d,stroke:#61dafb
    style Monitor fill:#2d2d2d,stroke:#98c379
```
...
flowchart LR
    ...
    style B1 fill:#2d2d2d,stroke:#e06c75
    style B2 fill:#2d2d2d,stroke:#e06c75
    style B3 fill:#2d2d2d,stroke:#e06c75
    style B4 fill:#2d2d2d,stroke:#e06c75
    style G1 fill:#2d2d2d,stroke:#98c379
    style G2 fill:#2d2d2d,stroke:#98c379
    style G3 fill:#2d2d2d,stroke:#98c379
    style G4 fill:#2d2d2d,stroke:#98c379
```
...
```mermaid
flowchart TB
    classDef epic fill:#2d2d2d,stroke:#e06c75
    classDef story fill:#2d2d2d,stroke:#e5c07b
    classDef task fill:#2d2d2d,stroke:#98c379
...
```mermaid
flowchart LR
    classDef gen fill:#2d2d2d,stroke:#e5c07b
    classDef verify fill:#2d2d2d,stroke:#98c379
    classDef fix fill:#2d2d2d,stroke:#e06c75
    classDef done fill:#2d2d2d,stroke:#61dafb
...
## 7. 🏗️ Vibe → Spec → Harness：工程成熟度演进路径

> 来源：面向工程团队的 Agent 时代软件开发架构框架。

在 AI 参与软件开发后，团队的核心矛盾已经不只是"代码写得够不够快"，而是三个新问题：
...
|------|------|----------|
| **Vibe** | 负责快速探索方向 | 探索能力 |
| **Spec** | 负责冻结设计共识 | 语义治理能力 |
| **Harness** | 负责约束执行过程和结果 | 执行治理能力 |

这三者不是互斥关系，而是**分层增强的演进路径**——允许探索，要求收敛，强制验证。
...
工程意图层 / Intent Layer
    （业务目标、用户需求、缺陷、变更请求）
                ↓
规格层 / Spec Layer
    （需求说明、设计规格、接口契约、数据模型、验收标准）
                ↓
约束层 / Harness Layer
    ├── 上下文约束：Agent 应该看到什么
    ├── 工具约束：Agent 可以做什么（白名单、权限边界）
    ├── 验证约束：结果对不对（单元测试、契约测试、静态检查）
    └── 治理约束：过程是否可控（代码审查、CI 门禁、审计）
                ↓
执行层 + 反馈层
    （Agent 执行 → 反馈回路 → Spec/Harness 更新）
```

**关键原则**：
- **意图不能直接裸奔到执行层** — 缺少 Spec 的意图会让 Agent 靠猜
- **Spec 是正式收敛层** — 把口头知识和模糊共识转成团队可对齐的约束
- **Harness 是执行约束层** — 让 Spec 有执行力，而不只是文档
- **Feedback 要回流** — 问题修复后沉淀为 Spec 补充或 Harness 新规则

### 五个成熟度阶段

| 阶段 | 表现 | 主要问题 |
|------|------|----------|
| **阶段一：纯 Vibe** | 直接开工，无正式边界 | 高度依赖个人经验，难并行，难回归 |
| **阶段二：Vibe + 零散 Spec** | 关键需求开始成文，但分散 | 规格已出现，但不足以成为统一依据 |
| **阶段三：规格驱动（SDD）** | 大部分开发先写需求、设计、验收 | 有定义，但还缺少强执行力 |
| **阶段四：Spec + Harness** | 关键路径有验证，Agent 工作进入可控状态 | 从"会描述"走向"会约束"的关键阶段 |
| **阶段五：反馈驱动** | 线上问题反向补规格，新成员能快速进入上下文 | 成熟工程团队的稳定状态 |

> 💡 **实用建议**：多数项目当前最现实的目标是先达到**阶段三（SDD 级别）**，让规格驱动开发成为默认工作方式；在此基础上，再针对高风险路径补充适当的 Harness 验证，而不是追求一步到位的重型治理。

### 最小落地顺序

1. **建立统一 Agent 入口**：明确从哪里开始读（CLAUDE.md/AGENTS.md），什么行为禁止，什么任务必须验证
2. **补最小 Spec 骨架**：需求 + 设计 + 契约 + 验收，让正式开发以 Spec 为起点
3. **先做高价值 Harness**：优先覆盖高频改动路径、高风险链路、曾出过事故的行为
4. **把问题沉淀成资产**：每次真实问题落成一条 Spec 补充或一条 Harness 验证
5. **季度级回顾**：哪些规格失效、哪些约束不跑、哪些 Agent 任务仍然高风险

### 常见反模式

| 反模式 | 表现 | 后果 |
|--------|------|------|
| **伪 Vibe** | 以探索为名长期逃避收敛 | 长期停留在低可维护状态 |
| **伪 Spec** | 文档很多，但无法裁决争议 | 仍靠人脑同步，文档只是装饰 |
| **纸面 Harness** | 有测试脚本，但默认不执行或结果不影响交付 | 约束体系变成展示材料 |
| **Agent 失控** | Agent 可以直接改很多文件，无边界无验证 | 生成速度越快，错误积累越快 |
...
[📚 返回目录](../../README.md#tutorial-contents) | [⬅️ 上一章：Ch08 扩展生态与会话管理](./ch08-config-session.md) | [➡️ 下一章：Ch10 人机协同方法论](./ch10-collaboration.md)
