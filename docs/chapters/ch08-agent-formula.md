...
- [0. 从 Ch07 的闭环，进入系统视角](#0-从-ch07-的闭环进入系统视角)
- [1. 一张总图：从四件套到三层坐标](#1-一张总图从四件套到三层坐标)
- [2. LLM：大脑，但不是整个系统](#2-llm大脑但不是整个系统)
- [3. Harness / Control Plane：真正的杠杆在模型外侧](#3-harness--control-plane真正的杠杆在模型外侧)
- [本章总结](#本章总结)

> 📖 **阅读方式建议**：如果你对 `LLM / Context / Token / MCP / Skill / Harness` 这些词还不熟，可以把 [基础概念与术语](./ch06-glossary.md) 另开一页，边读边查。
>
> 📌 **和 Ch07 的分工**：Ch07 讲了”为什么需要从 LLM 走到 Agent”——开环 vs 闭环、ReAct、自主性光谱。本章接着拆解”Agent 到底由哪些部件组成”。

---

## 0. 从 Ch07 的闭环，进入系统视角

Ch07 建立了一个关键认知：Agent 的核心是**反馈闭环**，而不是一次性回答。

本章接着回答下一个问题：**这个闭环系统到底由哪些部件组成？为什么同一个模型换一套工作流，表现会像换了一个系统？**

先记住两条容易踩的坑：

| 常见直觉 | 更接近现实的说法 |
|---|---|
| “工具越多越强” | 工具太多会增加上下文负担、选择噪音和风险面 |
| “Agent 效果主要看模型” | 效果更接近 `Model × Context × Task Structure × Verification`——模型只是其中一个变量 |
