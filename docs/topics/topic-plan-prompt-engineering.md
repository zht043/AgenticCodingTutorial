...
> 📌 **本章只回答一个问题**：怎样让 Agent 不抢跑？如果上一篇解决的是"让 Agent 看懂项目并会自证"，本篇解决的就是"怎样让它按你的节奏推进，而不是一上来把仓库改穿"。
...
flowchart LR
    ...
    E["探索 先理解现状，不改文件"]:::explore
    G1["检查点 1 确认理解没跑偏"]:::gate
    P["规划 只出 Plan，不执行"]:::plan
    G2["检查点 2 确认计划合理"]:::gate
    C["编码 按 Plan 执行并验证"]:::code
...
[📚 返回目录](../../README.md#tutorial-contents) | [🔍 代码探索与验证驱动](./topic-explore-verify-workflow.md)
