...
官方提供托管设置 `disableBypassPermissionsMode: "disable"` 来组织级封锁该模式。
...
2025–2026 年间，Claude Code 公开披露了 7 个 CVE/GHSA 漏洞，覆盖信任提示绕过、命令解析缺陷、路径限制绕过、域名验证缺陷等多类攻击向量。以下按严重程度排列。
...
### 2.6 CVE-2026-24052 · WebFetch 域名验证缺陷（CVSS: 未公开，CWE-601）（CVSS：未公开评分，CWE-601）
...
| 告警类型 | 触发条件 |
|---|---|
| 沙箱外执行 | `tool_result.source=config_auto` 且 `sandbox=false` |
