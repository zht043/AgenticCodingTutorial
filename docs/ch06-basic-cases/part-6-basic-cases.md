...
> 🎯 场景：一个较长的任务（如跨多个文件的重构或批量分析）开始出现前后矛盾、重复劳动的现象，你需要让 Agent 保持专注。

### 为什么需要上下文卸载

随着对话轮次增多，Agent 需要处理的历史信息成倍增长，容易出现：

- 前面说好的约束，后面被忘掉
- 重复做已经做过的事
- 成本失控（每轮都带着全量历史）

**根本原因**：你把所有中间状态都留在对话里，而不是写到文件里。
...
---
flowchart LR
    A["📋 阶段 1<br/>分析 & 产出摘要"] -->|写入文件| F["📄 phase1-summary.md"]
    F -->|新上下文读入| B["⚙️ 阶段 2<br/>执行，携带摘要"]
    B -->|写入文件| G["📄 phase2-log.md"]
    G -->|新上下文读入| C["✅ 阶段 3<br/>验证 & 汇总"]

    style A fill:#2d2d2d,stroke:#61dafb,color:#fff
    style B fill:#2d2d2d,stroke:#98c379,color:#fff
    style C fill:#2d2d2d,stroke:#c678dd,color:#fff
    style F fill:#2d2d2d,stroke:#e5c07b,color:#fff
    style G fill:#2d2d2d,stroke:#e5c07b,color:#fff
```
...
```
我要对 src/ 下的所有 API 接口做一次命名规范审查。
请把这个任务分成 3 个阶段并告诉我每个阶段的产出文件名，
不要一次性全做完。
```
...
```
第 1 阶段完成后，请把分析结果写入 /tmp/api-audit-phase1.md，
格式为：文件名 | 问题描述 | 建议改法。
写完告诉我文件路径，然后停下来等我确认。
```
...
```
请读取 /tmp/api-audit-phase1.md，
基于其中的问题清单，执行第 2 阶段：逐一修改。
每改完一个文件就追加一条记录到 /tmp/api-audit-phase2-log.md。
```
...
| 做法 | 效果 |
|------|------|
| 中间产物写入文件 | 上下文保持精简，成本可控 |
| 下一阶段只读摘要 | Agent 专注当前任务，不被历史干扰 |
| 按需加载原始详情 | 需要时再读完整文件，不是默认塞入 |

> ⚠️ **反模式**：让 Agent 在一次超长对话里完成所有阶段。对话越长，注意力越分散，越容易引入矛盾。

### 经验沉淀

```markdown
## 多阶段任务的上下文管理
- 每个阶段的产出写入独立文件（如 phase1-summary.md）
- 下一阶段启动时只携带上一阶段的摘要，不是全量历史
- 需要原始详情时再按需加载
```

> 💡 **进阶延伸**：当任务规模更大（需要数十步甚至上百步），需要配合外层循环驱动多个 Agent 实例时，参见[第 13 章 · 进阶案例 — 外层循环模式（Ralph Wiggum 模式）](../ch13-advanced-cases/part-13-advanced-cases.md)。
...
这就是 [Chapter 2](../ch02-concepts/part-2-concepts.md) 中提到的「上下文演化」在实践中的样子：
